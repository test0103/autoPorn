...
Put the backend Authorization token in your untracked local YAML file (`api.authorization`). `dry_run: true` still reads live API data for search, section lists and covers, but blocks write/mutation calls unless `--execute` is passed. Cover fetch failures and unsupported cover bytes, including CDN formats such as `.bnc` that cannot be decoded by Pillow, are retried and then downgrade to title/tag-only planning instead of rejecting the item.
...
The first run reads live API data and writes an Excel plan, but does not publish or re-classify while `dry_run: true`. If a cover URL keeps failing or returns bytes Pillow cannot identify, the row stays eligible based on title/tag classification and the reason contains `cover-fetch-failed` plus `title-only`. To execute real publishing and re-classification after auditing the plan:
